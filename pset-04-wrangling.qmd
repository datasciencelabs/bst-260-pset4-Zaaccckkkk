...
stopifnot(exists("census_key"), is.character(census_key), nzchar(census_key))
```
...
# Change the long region name to a shorter one
  mutate(
    region_name = if_else(
...
# Construct and perform the request, extract the data
...
```
:::Comment
This is not the full dataset. This endpoint is hosted on Socrata; by default Socrata applies a row limit of 1,000 when no limit is provided.
:::
...
cases <- cases_raw |>
  mutate(
    # Convert end_date into Date format
...
cases |>
  ...
  # Filter data in 2020 and 2021
  filter(year %in% c(2020, 2021)) |>
  # Join with population to get region names and population
  right_join(
    population |> select(state_abb, year, population, region_name),
    by = c("state" = "state_abb", "year" = "year")
  ) |>
  # Calculate cases per 100k
  mutate(cases_per_100k = (cases / population) * 1e5) |>
  # Plot cases vs time, group by region
  ggplot(aes(date, cases_per_100k, group = state, color = state)) +
  geom_line() +
  facet_wrap(~ region_name, ncol = 2) +
  labs(
    title = "Cases per 100,000 by State (2020–2021)",
    x = "Date", 
    y = "Cases per 100,000",
    color = "Region"
  )
...
```{r}
cases |>
  # Get the year and month from date
  mutate(year = year(date), month = month(date, label = TRUE, abbr = FALSE)) |>
  # Include cases from 2020 to 2021
  filter(year %in% c(2020, 2021)) |>
  # Calculate total cases by month and year
  group_by(year, month) |>
...
deaths <- request(deaths_url) |>
  ...
  req_perform() |>
  resp_body_json(simplifyVector = TRUE) |>
  # Convert date to Date format, deaths to integer format
  mutate(
    date = as.Date(end_date),
    deaths = as.integer(covid_19_deaths)
  ) |>
